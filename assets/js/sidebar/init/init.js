...
export function initEventListeners() {
  ...
  saveTextBlockButton.addEventListener("click", () => {
    ...
    if (title && text) {
      ...
      const textBlock = {
        id: textBlockID.value || new Date().getTime(), // Get the ID from hidden input
        title: title,
        text: text,
        folderId: folderId,
      }

      if (textBlockID.value) {
        textBlockManager.updateTextBlock(textBlock) // Update existing text block
        disableEditMode() // Disable edit mode after saving
      } else {
        textBlockManager.addTextBlock(textBlock) // Add new text block
      }
